...
// adds path to see list of game users in GameUserDB
@Path("/gameusers")
...
public class GameUserRESTController {
    ...
    // added role user to show list of users in game user DB
    @RolesAllowed({"USER"})
    ...
    public Response getGameUserById(@PathParam("id") Integer id,@Auth GameUser user) {
        ...
        if (gameUserInfo != null)
        	...
        	return Response.ok(GameUserDB.createGameUser(id)).build();
        }
    ...
    // added role admin to see list of users in game user DB
    @RolesAllowed({"ADMIN"})
